...
// Each product's roots. The rest of its target list is the transitive closure of these over the
// link graph recorded by Scripts/build-xcframeworks.sh -- `.binaryTarget` can't declare
// dependencies, so a product has to name every framework it needs, and hand-maintaining that list
// goes stale the moment Xcode changes how it links a SwiftPM dependency.
...
Prebuilt macOS XCFrameworks of [\(sourceRepo)](https://github.com/\(sourceRepo)). Depending
...
```swift
.product(name: "CodeEditSourceEditor", package: "\(identity)"),  // was "\(sourceIdentity)"
```
