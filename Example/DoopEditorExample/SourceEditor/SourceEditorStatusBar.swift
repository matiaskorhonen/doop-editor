...
struct SourceEditorStatusBar: View {
            ...
            Menu {
                ...
                Menu {
                    ...
                    Toggle(
                        "Warning Characters",
                        isOn: Binding(
                            get: { !warningCharacters.isEmpty },
                            set: { newValue in
                                if newValue {
                                    warningCharacters.insert(0x200B)
                                } else {
    ...
    @ViewBuilder private var scrollPosition: some View {
        HStack(spacing: 0) {
            ...
            TextField(
                "",
                value: Binding(
                    get: { Double(state.scrollPosition?.x ?? 0.0) },
                    set: { state.scrollPosition?.x = $0 }
                ),
                formatter: formatter
            )
            ...
            TextField(
                "",
                value: Binding(
                    get: { Double(state.scrollPosition?.y ?? 0.0) },
                    set: { state.scrollPosition?.y = $0 }
                ),
                formatter: formatter
            )
            .textFieldStyle(.plain)
    ...
    func getLabel(_ cursorPositions: [CursorPosition]?) -> String {
        ...
        if cursorPositions.isEmpty { return "No cursor" }
        if cursorPositions.count > 1 { return "\(cursorPositions.count) selected ranges" }
