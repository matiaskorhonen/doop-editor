...
extension CGContext {
    ...
    ///
    /// `internal` is enough now that the gutter lives in this module too. Keeping the Obj-C
    /// shim to a single module is what lets the binary distribution absorb it rather than ship
    /// it as its own framework -- see BINARY_DISTRIBUTION.md.
    ///
